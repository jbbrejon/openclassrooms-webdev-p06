...
exports.signup = (req, res, next) => {
    bcrypt.hash(req.body.password, 10) // Call "hash() method from bcrypt module" (arguments : password, rounds)
        .then(hash => {
            const user = new User({
            ...
            user.save() // Call "save()" method from mongoose module
                .then(() => res.status(201).json({ message: 'User created!' })) // if response contains the status code "201" -> return confirmation message
                .catch(error => res.status(400).json({ error })); // if response contains status code "400" -> return error message
        })
        .catch(error => res.status(500).json({ error })); // Display error if user not created
};

// Set login commands
exports.login = (req, res, next) => {
    User.findOne({ email: req.body.email }) // Call "findOne() method from mongoose module to check if the user exists"
        .then(user => {
            if (!user) {
                ...
                .then(valid => {
                    ...
                    res.status(200).json({
                        ...
                        token: jwt.sign(
                            ...
                            tokenString,
                            { expiresIn: '24h' }
                        ) // sign() method of jsonwebtoken (arguments : user's id from MongoDB, token string, expiration)
